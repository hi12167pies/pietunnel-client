...
async function main() {
  ...
  let token = CLI.get("token")

  try {
    if (token == null) {
      // Get token from the token file
      const tokenFile = await fs.readFile(path.join(process.cwd(), "pietunnel_token.txt"), 'ascii')
      token = tokenFile.slice(tokenFile.indexOf("token=") + "token=".length)
    }
  } catch (e) {
    // If error re-write the file
    console.log(`Error reading pietunnel_token.txt, ${e.message}`)
    console.log(`Writing new default file.`)
    await fs.writeFile(path.join(process.cwd(), "pietunnel_token.txt"), "token=")
    process.exit()
  }
